...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener
{
  ...
  private JPanel createColorInfoPanel(Font labelFont)
  {
    ...
    int b = pixel.getBlue();
    if (r < 10)
      rValue = new JLabel("R:   " + r);
    else if (r < 100)
      rValue = new JLabel("R:  " + r);
    else
      rValue = new JLabel("R: " + r);
    if (g < 10)
      gValue = new JLabel("G:   " + g);
    else if (g < 100)
      gValue = new JLabel("G:  " + g);
    else
      gValue = new JLabel("G: " + g);
    if (b < 10)
      bValue = new JLabel("B:   " + b);
    else if (b < 100)
      bValue = new JLabel("B:  " + b);
    else
      bValue = new JLabel("B: " + b);
...
//                               infoPanel.getFont().getStyle(),14);
    Font largerFont = new Font("Courier New",
                               Font.BOLD,14);
  ...
  {
    Picture pix = new Picture("APCSP1.jpg");
//    pix.faceComposite();
    //pix.grayscaleWithLuminance();
    pix.grayscale();
   //pix.faceComposite();
   // pix.brighten();
    // pix.colorize();
